bd_est <- function(data, magic_weight_ps, magic_weight_outcome){
  ...
  ps_model <- glm(X2~X1, data = data, family = binomial(), weights=magic_weight_ps)
  
  # outcome_model <- glm(X3~I(X1*X2)+X2, data = data, family = gaussian(), weights=magic_weight_outcome)
  outcome_model <- glm(X3~X1+X2, data = data, family = gaussian(), weights=magic_weight_outcome)
  ...
  ATE <- est_a1 -  est_a0
  IF_ATE <- IF_a1 - IF_a0
  var_ATE <- mean(product_NA0(IF_ATE^2, magic_weight_outcome)) / nrow(data)
